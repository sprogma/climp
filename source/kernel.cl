...
float StrangeSound(float s, struct note *note, float rnd){ 
    float v = note->volume;
...
float PianoChord(float s, struct note *note, float rnd){ 
    float freq[] = {
            1.0,
        0.5,
        0.2,
...
float PianoChrodBass(float s, struct note *note, float rnd){ 
...
)
{
    ...
    while ((n = opt[beat * PARALLEL_CHANELS + note]) != -1)
    {
        if (notes[n].start <= s && s <= notes[n].end)
        {
            switch (notes[n].tool)
            {
            case 0: res += StrangeSound(s, notes + n, rnd); break;
case 1: res += PianoSolo(s, notes + n, rnd); break;
case 2: res += PianoChord(s, notes + n, rnd); break;
case 3: res += PianoBass(s, notes + n, rnd); break;
case 4: res += PianoChrodBass(s, notes + n, rnd); break;
