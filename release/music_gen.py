...
class SynthesizerProject:
    ...
    def create(self, what, dt=0.25, X=False):
        ...
        def tact(time=None):
            self.tacts.append(SynthesizerProjectTact(time, len(self.configs.kernel.tools)))
        ...
        def whispers(instrument_edition=False):
            ...
            if not instrument_edition:
                ...
                self.configs.kernel.tools.append(SynthesizerTool(name="Violin", code="""
                        ...
                        return res;
                """.replace(" " * 12, "")))
                #self.configs.kernel.tools.append(SynthesizerTool.from_wave(lambda x: self.log(x, c.log.info), "Alto", "test2.mp3", 1.0, L=64, bass_boost=5.0, distortion=100.0))
                ...
                self.configs.kernel.tools[0].configs.volume = 0.25 * 0.65
                self.configs.kernel.tools[1].configs.volume = 0.25 * 0.65
                self.configs.kernel.tools[2].configs.volume = 0.16 * 0.65
                self.configs.kernel.tools[3].configs.volume = 0.1 * 0.65
                self.configs.kernel.tools[4].configs.volume = 0.14 * 0.65
            ...
            def mytact(s):
                t = 0.0
                # for all lines
                self.tacts.append(SynthesizerProjectTact(t,len(self.configs.kernel.tools)))
                prev_num = -1
                ...
                for line in map(str.strip, s.split('\n')):
                    ...
                    if line and line[0].isdigit():
                        ...
                        num = int(num)
                        if num < prev_num:
                            t += self.configs.tact_size # new tact
                            self.tacts.append(SynthesizerProjectTact(t,len(self.configs.kernel.tools)))
                            cnt = defaultdict(int)
                        prev_num = num
                        ...
                        for note in content.split():
                            ...
                            if cnt[num] >= len(self.tacts[-1].notes[num]):
                                self.tacts[-1].insert_column(len(self.tacts[-1].notes[num]))
        ...
        def test():
            ...
            self.configs.kernel.tools.append(SynthesizerTool(name="Test1", code="""
                    ...
                    return res;
            """.replace(" " * 12, "")))
            self.configs.kernel.tools.append(SynthesizerTool.from_wave(lambda x: self.log(x, c.log.info), "Test2", "test3.mp3", 2.0, bass_boost=1.0, L=16, distortion=None))
